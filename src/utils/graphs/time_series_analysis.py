...
def get_mean_variance(dataset, analysis_variable, n_chunks=20):
	...
	df_unique_station_chunks = np.array_split(dataset[analysis_variable],
											  n_chunks)
	...
	if isinstance(dataset, pd.DataFrame):
		if analysis_variable in dataset.columns:
			...
			mean_global, variance_global, means, variances = get_mean_variance(
				dataset=dataset, analysis_variable=analysis_variable,
				n_chunks=n_chunks
			)
...
def test_ljung_box(data, n_lags=10):
	...
	if not result_test_ljung_box[
		result_test_ljung_box["lb_pvalue"] > 0.05].empty:
