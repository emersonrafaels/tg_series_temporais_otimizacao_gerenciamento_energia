    ...
    try:
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if isinstance(groupby_column, str):
                ...
                list_columns_to_select = [groupby_column, column_x_axis, column_y_axis]

                data = select_columns_dataframe(
                    data=data, list_columns_to_select=list_columns_to_select
                )

                # VERIFICANDO SE A COLUNA DE GROUPBY ESTÁ DENTRO DOS DADOS
                if groupby_column in data.columns:
                    data_group = (
                        data.groupby(by=[groupby_column])
