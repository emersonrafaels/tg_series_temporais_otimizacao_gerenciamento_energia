...
def load_dataset_ghcn(dataframe_return):
	...
	with st.expander(label="Metadados"):
		...
		st.text('month - Representa o mês da medição.\nDe 1 a 12, cada valor representa um dos meses do ano')
		st.text('day - Representa o dia da medição.\nDe 1 a 31, cada valor representa um dos dias do mês')
		st.text(
			'stn - Fornece o id da estação meteorológica')
		st.text(
			'name - Fornece o nome da estação meteorológica')
		st.text(
			'country - Fornece o país da estação meteorológica\n(no caso, todos os dados referem-se ao Brasil)')
		st.text(
			'city - Fornece a cidade da estação meteorológica')
		st.text(
			'state - Fornece o estado da estação meteorológica')
		st.text(
			'lat - Latitude no qual a estação meteorológica está localizada')
		st.text(
			'lon - Longitude no qual a estação meteorológica está localizada')
		st.text(
			'begin - Data de início das medições na estação metereológica')
		st.text(
			'end - Data de fim das medições na estação metereológica')
		st.text(
			'elev - Altura em média no qual a estação metereológica\nestá localizada, em relação ao nível do mar (m)')
		st.text(
			'temp - Temperatura média (C°)')
		st.text(
			'prcp - Precipitação média (Pa)')
		st.text(
			'wdsp - Velocidade média do vento (m/s)')
		st.text(
			'date_measurement - Valor em formato data que\nrepresenta o ano e mês dos dados medidos')

	# QUANTIDADE DE DADOS SELECIONADOS
	st.text(
		"Quantidade de medições: {}".format(
	...
	validator_map, st.session_state[
		"mapobj"], _ = plot_map_dataset_ghcn(
	...
	options_group = st.session_state["selected_df"][
		st.session_state[
			"filter_dataset_group"]].sort_values().unique()
	...
	dataframe_plot = st.session_state["selected_df"][
		st.session_state["selected_df"][
			st.session_state["filter_dataset_group"]].isin(
	...
	with st.expander("Temperatura"):
		...
		container_temp.markdown(
			"## 2. Analisando a curva - White Noise")
